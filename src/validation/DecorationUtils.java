...
//        Bounds targetBounds = targetNode.getBoundsInLocal();  // ensure position will be right after the transition is applied. it caused a shift of the decoration when the field gets focus.
        ...
        // position
        switch (pos) {
			case TOP_CENTER:		x += targetWidth / 2;		break;
			case TOP_RIGHT:			x += targetWidth;			break;
			case CENTER_LEFT:		y += targetHeight / 2;		break;
			case CENTER:			x += targetWidth / 2;		y += targetHeight / 2;				break;
			case CENTER_RIGHT:		x += targetWidth;			y += targetHeight / 2;				break;
			case BOTTOM_LEFT:		y += targetHeight;			break;
			case BOTTOM_CENTER:		x += targetWidth / 2;		y += targetHeight;				break;
			case BOTTOM_RIGHT:		x += targetWidth;			y += targetHeight;				break;
			case BASELINE_LEFT:		y += baselineOffset - decorationNode.getBaselineOffset() + height / 2;				break;
			case BASELINE_CENTER:   x += targetWidth / 2;   	y += baselineOffset - decorationNode.getBaselineOffset() + height / 2;    break;
            case BASELINE_RIGHT:    x += targetWidth;  			y += baselineOffset - decorationNode.getBaselineOffset() + height / 2;      break;
        }
    ...
    public static void setTargetDecoratorCleaned(Node targetNode, boolean clean) {
        ...
        if (clean)       targetNode.getProperties().put(PROPERTY_TARGET_DECORATOR_STATUS, PROPERTY_TARGET_DECORATOR_STATUS_CLEANED);
        else             targetNode.getProperties().remove(PROPERTY_TARGET_DECORATOR_STATUS);
    ...
    public static void setAnimationPlayed(Node decorateNode, boolean played) {
        if (decorateNode == null)             return;
        if (played)     decorateNode.getProperties().put(PROPERTY_DECORATOR_ANIMATION_STATUS, PROPERTY_DECORATOR_ANIMATION_STATUS_PLAYED);
        else  			decorateNode.getProperties().remove(PROPERTY_DECORATOR_ANIMATION_STATUS);
