...
public class NumberField extends TextField
{
        ...
        public NumberField(BigDecimal value, NumberFormat nf) {
            super();
            this.nf = nf;
            setStyle("-fx-text-alignment:RIGHT;");			// NOOP  ???
            initHandlers();
            setNumber(value);
        }
...
// try to parse when focus is lost or RETURN is hit
            setOnAction(new EventHandler<ActionEvent>() {
                @Override public void handle(ActionEvent arg0) {    parseAndFormatInput();     }
            });
...
// Set text in field if BigDecimal property is changed from outside.
            numberProperty().addListener(new ChangeListener<BigDecimal>() {
                @Override public void changed(ObservableValue<? extends BigDecimal> obserable, BigDecimal oldValue, BigDecimal newValue) {
