...
public class SlideEffectDemo extends Application
{
	...
	private void configureBox(VBox root) {
		...
		bottomPane.setBackground(new Background(new BackgroundFill(lg, CornerRadii.EMPTY, Insets.EMPTY)));
		bottomPane.getChildren().addAll(new Label("Click the above \"Slide Down\" button to see the top pane content..."));
		...
		sp1.getChildren().addAll(new Text("Click the below \"Slide Up\" button to see the bottom pane content..."));
		
		topPane = new StackPane();
		topPane.getChildren().addAll(sp1);
	...
	private HBox getActionPane()
	{
		...
		upBtn.setOnAction(arg0 -> { timelineUp.play();	});
		Button downBtn = new Button("Slide Down");
		downBtn.setOnAction(arg0 -> { timelineDown.play();	});
